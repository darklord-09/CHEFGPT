...
| **Column** | **Data Type** |
| `id` | `INT PRIMARY KEY` |
| `user_id` | `INT NOT NULL` (Foreign Key referencing `users_table`) |
...
## 🍽️ Recipe Application Structure (Frontend)

This outlines the structure for a web application built using **ReactJS** for the frontend, **Bootstrap** and **CSS** for styling.

### 🔑 Authentication

The application includes standard user authentication flows.

#### Sign-Up Page
* **Username Field:** Text input for creating a unique username.
* **Password Field:** Password input for setting an account password.

#### Sign-In Page
* **Username Field:** Text input for entering the registered username.
* **Password Field:** Password input for entering the account password.

---

### 💻 Dashboard

The main user interface upon successful sign-in. The user's ID is saved in **local storage** upon sign-in and used to fetch user-specific data.

#### LLM Recipe Generation Area (Based on Ingredients)

This section allows users to generate recipes based on ingredients they input.

* **Item Listing Area:**
    * Dynamic text fields to list ingredients.
    * **Add Field Button:** To add a new ingredient input field.
    * **Delete Button:** To remove an adjacent ingredient input field.
* **Submit Button:** Submits the listed ingredients to the **Large Language Model (LLM)**.
* **LLM Output Display Area:** A text area where the recipe generated by the LLM is displayed.

#### Manual Recipe Adder Area

This section allows users to manually save a complete recipe.

* **Recipe Name Field:** Text input for the title of the recipe.
* **Recipe Contents Field:** Text area for the ingredients and instructions.
* **Submit Button:** Saves the recipe to the database. Upon submission, only the **Display Area for Saved Items** is refreshed.

#### Search Bar for Recipes

This section provides a quick way to get a recipe from the LLM
